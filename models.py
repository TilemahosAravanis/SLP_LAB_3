...
class BaselineDNN(nn.Module):
    ...
    def __init__(self, output_size, embeddings, trainable_emb=False):
        ...
        self.Embeddings.weight = nn.Parameter(torch.from_numpy(embeddings)) # learnable parameter of the model # EX4

        # 3 - define if the embedding layer will be frozen or finetuned
        if not trainable_emb:
            self.Embeddings = self.Embeddings.from_pretrained(torch.Tensor(embeddings), freeze = True) # EX4
        ...
        self.output = nn.Linear(200, output_size)  # EX5
    ...
    def forward(self, x, lengths):
        ...
        Sentence_Embeddings = self.Embeddings(x) # EX6
        
        # 2 - construct a sentence representation out of the word embeddings
        representations = torch.sum(Sentence_Embeddings, dim=1)
